...
// Respect reduced motion for hero background
...
if (bookingForm && formMessage) {
  bookingForm.addEventListener("submit", (e) => {
    ...
    if (honeypot) {
      formMessage.textContent =
        "Spam detected. Your request could not be submitted.";
...
// Create optimized stars
function createOptimizedStars(sky, count = 80) {
...
function createOptimizedShootingStar(sky) {
  ...
  shootingStar.style.setProperty(
    "--shoot-x",
    `${length * Math.cos((angle * Math.PI) / 180)}px`
  );
  shootingStar.style.setProperty(
    "--shoot-y",
    `${length * Math.sin((angle * Math.PI) / 180)}px`
  );
